# This is the task for GEN72 arm + inspire hand to grap one lego block
# Forked from inspire_grasp_block_v2.py 
...
class InspireGraspBlockV3(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless, agent_index=None, is_multi_agent=None):
        ...
        # print("\033[40m检视参数task:randomize:" + str(self.randomize) + "\033[0m")
        ...
        self.arm_hand_default_dof_pos[:7] = to_torch([0.0, 0.45, 0.0, -1.0, 0.0, 0.5, 0.0] , dtype=torch.float, device=self.device)     
        ...
        self.arm_hand_prepare_dof_pos[:7] = to_torch([0.0, 0.45, 0.0, -1.0, 0.0, 0.5, 0.0] , dtype=torch.float, device=self.device)
        ...
        self.lego_lift_height = torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
    ...
    def _create_envs(self, num_envs, env_spacing, num_envs_per_row):
        ...
        arm_hand_asset_file = "mjcf/realman_gen72/gen72_inspire.xml"
        ...
        for env_id in range(num_envs):
            ...
            ## block setting option move here
            # =================== Set up the block =================== #
            lego_path = "urdf/blender/urdf/"
            lego_files = ['1x2.urdf', '1x2_curve.urdf', '1x3_curve_soft.urdf', '1x3_curve.urdf', '1x1.urdf', '1x3.urdf', '1x4.urdf', '2x2_curve_soft.urdf', '2x2.urdf', 'apple.urdf', 
                          'ball.urdf', 'biscuit.urdf', 'biscuit1.urdf', 'biscuit2.urdf', 'biscuit3.urdf', 'biscuit4.urdf', 'bottle.urdf', 'cheese.urdf', 'chocolate.urdf', 'clock.urdf', 
                          'coke.urdf', 'cookie.urdf', 'flashlight.urdf', 'JewelleryBox.urdf', 'meat_can.urdf', 'nectar.urdf', 'powerbank.urdf', 'remotecontroller.urdf', 'salt.urdf', 'stapler.urdf',
                          'yogurt.urdf']
            lego_file_name = lego_files[random.randint(0, 6)]
            ...
            # lego_start_pose.p = gymapi.Vec3(random.uniform(0.08, 0.28), random.uniform(-0.1, 0.1), random.uniform(0.6188, 0.8))
            lego_start_pose.p = gymapi.Vec3(random.uniform(0.23, 0.43), random.uniform(-0.1, 0.1), random.uniform(0.6188, 0.8))
            lego_start_pose.r = gymapi.Quat().from_euler_zyx(random.uniform(-1.57, 1.57), random.uniform(-1.57, 1.57), random.uniform(-1.57, 1.57))
            # =================== the block End =================== #
    ...
    def compute_reward(self):
        ...
        # 打印当前乐高高度
        # print("乐高初始高度：" + str(self.lego_start_pos[0,2].item()))
        # print("乐高当前高度：" + str(self.lego_pos[0,2].item()))
        # print("乐高高度差：" + str(self.lego_pos[0,2].item() - self.lego_start_pos[0,2].item()))
        self.lego_lift_height = self.lego_pos[:,2] - self.lego_start_pos[:,2]
        # print(self.lego_lift_height)
    ...
    def reset_envs(self, env_ids):
        ...
        # print("\033[41m检视参数reset_lego_index: " + str(reset_lego_index) + "\033[0m")
        if self.randomize:
            # print("\033[41m检视参数self.randomize: " + str(self.randomize) + "\033[0m")
    ...
    def pre_physics_step(self, actions):
        ...
        '''This part try to add some perturbation'''
        
        perturbation_on  = torch.where(self.lego_lift_height > 0.1, torch.ones_like(self.lego_lift_height), torch.zeros_like(self.lego_lift_height)).to(self.device)
        
        # print(perturbation_on)

        perturbation_random_amp = 0.5 * (torch.randn(self.num_envs).to(self.device) - 2 )
        # print(perturbation_random_amp)
        
        apply_perturbation = torch.mul(perturbation_on, perturbation_random_amp)
        # print(apply_perturbation)

        forces = torch.zeros((self.num_envs, 24, 3), device=self.device, dtype=torch.float)
        forces[:, -1, 2] = apply_perturbation
        # forces[:,1,2] = -10
        # forces[:,2,2] = -10
        # forces[:,3,2] = -10
        # forces[:,4,2] = -10
